...
-- Admin-controlled RemoteEvent damage (client-only visualization)
Modules.RemoteEventModule = (function()
    local M = {}
    local remote = Instance.new("RemoteEvent")
    remote.Name = "ClientDamageEvent"
    remote.Parent = game:GetService("StarterPlayer"):WaitForChild("StarterPlayerScripts")

    local damage = 50
    local radius = 10

    function M.SetDamage(dmg) damage = math.max(0, dmg) end
    function M.SetRadius(r) radius = math.max(1, r) end

    function M.Fire(origin)
        origin = origin or HRP.Position
        -- Visual effect only, client-side
        ...
        -- Optionally fire event for server handling if desired
        remote:FireServer(damage, radius, origin)
    end
...
-- Hollow Purple Damage Event
do
	UIS.InputBegan:Connect(function(input, processed)
		if processed then return end
		if input.KeyCode == Enum.KeyCode.Z then
			-- Fire RemoteEvent damage
			Modules.RemoteEventModule.Fire(HRP.Position + Vector3.new(0,3,0))
			-- Keep Hollow Purple visuals if desired
			if not Modules.HollowPurpleModule.IsCharging() then
				Modules.HollowPurpleModule.StartCharge()
			end
		end
